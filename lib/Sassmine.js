...
use('sassmine').on(function(sas) {
	...
	sas.Sassmine = Class.extend({
		...
		printer: new (typeof window == 'undefined' ? sas.ConsolePrinter : sas.DomPrinter),
		...
		start: function() {
			this.runTest(this.levels[0]);
		},

		runTest: function(test) {
			for (var i = 0, len = test.queue.length; i < len; i++)

		}

		runTestRecursive: function(test, index) {
			test.queue[index].run()
				.onDone(function() {
		...
		addTest: function(type, node) {
			if (this.running)
				this.levels[this.current].add(type, node);
			else
				node.execute();
		},
		...
		describe: function(message, code) {
			this.addTest(sas.TestType.SUITE, new sas.Suite(message, code));
		},

		it: function(message, code) {
			this.addTest(sas.TestType.SPEC, new sas.Spec(message, code));
		},
